...
def aggregate_by_gene_region(df):
    ...
    df_grouped = df.groupby(['geneChr', 'geneStart', 'geneEnd'])[
        'mean_methylation_difference'].mean().reset_index()
    df_grouped['region'] = df_grouped.apply(
        lambda row: f"{row['geneChr']}:{row['geneStart']}-{row['geneEnd']}", axis=1)
...
for idx, df in enumerate(aggregated_data):
    ...
    df_pivot = df.pivot_table(
        index='region', values='mean_methylation_difference')
    heatmap_data[f'BC0{idx+2}'] = heatmap_data.index.map(
        df_pivot['mean_methylation_difference'].to_dict())

heatmap_data = heatmap_data.fillna(0).reset_index().melt(
    id_vars='index', var_name='Sample', value_name='Mean_Methylation_Difference')
...
heatmap = alt.Chart(heatmap_data).mark_rect().encode(
    ...
    y=alt.Y('index:N', title='Generegions'),
    color=alt.Color('Mean_Methylation_Difference:Q',
                    scale=color_scale, title='Mean Methylation Difference')
).properties(
